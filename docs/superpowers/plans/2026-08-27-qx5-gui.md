...
shared `usb_transport.py`), PIL/Pillow, OpenCV
...
unchanged — it already imports the shared `IsoPump` and backend helper from
`usb_transport`.
...
**Interfaces:**
- Consumes:
  - `usb_transport.IsoPump(pyusb_dev, endpoint_addr, packet_size, packets=32, nurbs=8)`
    with `.start()`, `.stop()`, `.take() -> bytes`
  - `usb_transport.find_libusb_dll() -> str`
...
- Standalone `.exe` packaging is intentionally out of scope; use the
  repository `.venv` launcher.
