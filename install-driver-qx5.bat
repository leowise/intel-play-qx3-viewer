...
title Digital Blue QX5 - USB Driver Setup
cd /d "%~dp0"
...
$Inf  = Join-Path $Root "drivers\qx5_winusb.inf"
$HwId = "USB\VID_093A&PID_050F"

Write-Host "============================================"
Write-Host " Digital Blue QX5 - automatic WinUSB setup"
Write-Host "============================================"
Write-Host ""
...
[Standard.NTamd64]
%DeviceName% = USB_Install, USB\VID_093A&PID_050F
...
[Strings]
ManufacturerName = "Intel Play QX5 Open Source"
DeviceName = "Digital Blue QX5 Microscope"
'@

if (-not (Test-Path $Inf)) {
    $Inf = Join-Path $env:TEMP "qx5_winusb.inf"
    Set-Content -LiteralPath $Inf -Value $infText -Encoding ASCII
}

function Get-Qx5Device {
    Get-PnpDevice -ErrorAction SilentlyContinue |
        Where-Object { $_.InstanceId -like "*VID_093A&PID_050F*" }
}

Write-Host "Looking for the microscope (USB 093A:050F)..."
$dev = Get-Qx5Device
if (-not $dev) {
...
if ($dev) {
    ...
    Write-Host "WARNING: Device not visible yet. The driver will still be bound to 093A:050F."
...
try {
    $ok = [Qx5Driver]::UpdateDriverForPlugAndPlayDevices(
        [IntPtr]::Zero, $HwId, $infFull,
        [Qx5Driver]::INSTALLFLAG_FORCE, [ref]$reboot)
} catch {
    $ok = $false
}
...
if (-not $ok) {
    try {
        $ok = [Qx5Driver]::DiInstallDriver([IntPtr]::Zero, $infFull, 0, [ref]$reboot)
    } catch {
        $ok = $false
    }
}
...
try {
    ...
    Write-Host "Starting Zadig. Select the QX5 / 1.3M PC-CAM (093A:050F), choose WinUSB, click Install Driver."
    Start-Process $zadig
} catch {
    Write-Host "Could not download Zadig. Open https://zadig.akeo.ie/ and install WinUSB for 093A:050F."
